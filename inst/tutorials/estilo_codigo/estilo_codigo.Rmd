---
title: "ixpantia sesion_extra: Estilo de código"
output: 
  learnr::tutorial:
...
```{r long}
question("¿Cuál (es) de estos comandos es (son) correcto (s)?",
  answer("nuevo_df <- mi_df %>% mutate(var2 = var2 + 1) %>%  select(var1, var2, cat) %>% group_by(cat) %>% summarise(vars = var1 + var2) %>% arrange(vars) %>% mutate(var_nueva = vars + 2) %>% select(var_nueva)") ,
  answer("nuevo_df <- mi_df %>% 
         mutate(var2 = var2 + 1) %>%  
         select(var1, var2, cat) %>% 
         group_by(cat) %>% 
         summarise(vars = var1 + var2) %>% a
         rrange(vars) %>% 
         mutate(var_nueva = vars + 2) %>% 
         select(var_nueva)", correct = TRUE)
)
```
