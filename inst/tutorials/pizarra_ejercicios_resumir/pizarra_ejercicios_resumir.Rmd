...
En el set de datos **directores** tenemos información sobre directores de cine famosos y su nacionalidad, mientras que en **peliculas** tenemos información sobre películas conocidas y su director. Ahora una estos dos sets de forma que podamos calcular el número de películas de cada director. 
```{r A, exercise = TRUE}
...
grade_result(
  pass_if(~ identical(unlist(.result), 
                      unlist(inner_join(peliculas, premios))) , "Excelente!")
)
```
...
question("¿Qué resulta de un `full_join` entre directores y peliculas?",
  ...
  answer("Un set de datos con las observaciones de directores que coinciden en películas"),
  answer("Un set de datos con las observaciones de películas que coinciden en directores"),
  answer("Un set de datos que contiene la información de ambos sets", correct = TRUE),
  incorrect = "Incorrecto",
  correct = "Correcto")
```
