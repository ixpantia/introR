...
grade_result(
  pass_if(~ identical(unlist(.result),
                      unlist(mean(LifeCycleSavings$dpi))),
          "¡Genial!")
  )
```
...
grade_result(
  pass_if(~ identical(unlist(.result),
                      unlist(max(LifeCycleSavings$pop15))),
          "¡Buenísimo!")
  )
```
...
grade_result(
  pass_if(~ identical(unlist(.result),
                      unlist(min(LifeCycleSavings$pop75))),
          "¡Muy bien!")
  )
```
...
En el set de datos hay una variable que se llama dpi, que representa el ingreso
disponible per cápita, ahora cambie el nombre de esta variable a
`diposable_income`.
...
grade_result(
  pass_if(~ identical(unlist(.result),
            unlist(colnames(LifeCycleSavings)[4] <- "disposable_income")),
            "Bien hecho!")
  )
```
...
```{r suma-check}
grade_result(
  pass_if(~ identical(unlist(.result), unlist(
          LifeCycleSavings$suma <- LifeCycleSavings$pop15 + LifeCycleSavings$pop75)
          ), "Bien hecho!")
  )
```
...
grade_result(
  pass_if(~ identical(unlist(.result), 
            unlist(subset(LifeCycleSavings, LifeCycleSavings$pop15 < 23 | LifeCycleSavings$pop75 > 47))),
          "Bien hecho!")
)
```
...
grade_result(
  pass_if(~ identical(unlist(.result),
            unlist(top_n(LifeCycleSavings, 6, pop75) %>%
