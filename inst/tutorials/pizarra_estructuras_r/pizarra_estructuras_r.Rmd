---
title: "ixpantia: Estructuras en R pizarra"
...
Este es un set de datos donde cada observacion se refiere a una region. Contiene 4 observaciones y 4 variables (columnas).
...
El set de datos regiones tiene 4 filas, ahora obtenga los datos que se encuentran en la tercera fila, incluyendo todas las columnas.
...
El de datos tiene 4 columnas, ahora obtenga la información de la cuarta columna, incluyendo todas las filas. 
...
En el set de datos hay una columna llamada *region* que se refiere al punto cardinal al que pertenece la región: NORTE, SUR, ESTE u OESTE. Ahora obtenga la información referente a aquellas regiones que pertenezcan a la region NORTE solamente. 
...
Este set de datos contiene informacion sobre el cilo de vida de los ahorros de 50 paises, inspirado en la hipótesis de Franco Modigliani sobre la razón de ahorro: esta puede ser explicada por el ingreso disponible per capita, el porcentaje de cambio en el ingreso disponible per capita, el porcentaje de población con menos de 15 años y el porcentaje de población con más de 75 años. Los datos fueron recolectados en la década 1960-1970. 
```{r}
...
| `pop75`      | % de poblacio con más de 75 años    |
...
```{r med_dpi-check}
grade_result(
  pass_if(~ identical(unlist(round(.result, 0), 1107)), "Custom message for value 2.")
)
```
...
En el set de datos hay una variable que representa el porcentaje de pblación que es menor de 15 años, descubra cuál país tiene un mayor porcentaje de esta población. 
...
En el set de datos hay una variable que representa el porcentaje de pblación que es mayor de 75 años, descubra cuál país tiene un menor porcentaje de esta población. 
...
```{r pob15_2-check}
grade_result(
  pass_if(~ identical(unlist(.result), unlist(min(life_savings$pop15))), "Muy bien!")
)
```


### 4. Hacer que los nombres de las filas se conviertan en una columna 

Las filas están identificadas con el nombre del país, convierta estos nombres en una nueva columna llamada "paises"

```{r row_names, exercise=TRUE, exercise.eval = FALSE}
...
```{r row_names-check}
grade_result(
  pass_if(~ identical(unlist(.result), unlist(LifeCycleSavings$paises <- row.names(LifeCycleSavings))), "Buenisimo!")
)
```


### 5. Cambiar nombre dpi a disposable_income
...
### 6. ¿Por qué no es recomendable hacer subsets por posición?
...
### 7. Generar nueva columna que contenga la suma de pop15 y pop75 
...
```{r suma-hint-1}
"LifeCycleSavings$suma o mutate"
```
...
### 8. Subset con países que tengan porcentajes de población menores a 15 años  menor 23% y mayor a 47%

El set de datos incluye información de 50 países, cada uno con un porcentaje de población menor de 15 años (pop15) y un porcentaje de población mayor 75 años (pop75). Ahora obtenga un subconjunto donde solo se encuentren los países en los que pop15 sea menor a 23% y pop75 mayor a 47%.
...
```{r porc-check}
grade_result(
  pass_if(~ identical(unlist(.result), unlist(subset(LifeCycleSavings, LifeCycleSavings$pop15 < 23 | LifeCycleSavings$pop15> 47))), "Bien hecho!")
)
```


### 9. Obtener un subset de los 6 países con mayor porcentaje de población mayor a 75 
