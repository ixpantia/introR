...
Este set de datos contiene datos sobre distintos superheroes y villanos de distintas compañias. Contiene 7 observaciones y 4 variables. 
```{r}
glimpse(superheroes)
```
...
### Haga un join de forma que aparezca el editor y el año de fundacion de los superheroes, pero solo de aquellos que tienen un editor en la base editores

En el set de datos **superheroes** tenemos datos sobre distintos superheroes, incluyendo la compañia de donde provienen, mientras que en **editores** tenemos información sobre las compañias en sí. Utilice alguno de los `joins` para unir ambas bases de datos de forma que se obtenga una nueva con la información de los superhéroes, su editor y el año de fundación de este. Si un superhéroe tiene un editor que no aparece en **editores** debe ser excluido.
...
### Haga un join de forma que a los datos de **superheroes** se les agrege la columna `anio_fundacion' sin excluir ningún superhéore o villano

Anteriormente hicimos un `join` que agregaba el año de fundacion de los editores, excluyendo a aquellos personajes que no tenían un editor con datos en **editores**, ahora debe hacer otro join que no excluya a ningún personaje al hacer el join y agregar la columna de `anio_fundacion`.
...
### Haga un join de tal forma que resulte una base que excluya a los superheroes sin editor en **editores** pero que no agregue `anio_fundacion`
...
### Haga un join de tal forma que resulte una base son aquellos superheores que tienen un editor que **NO** aparece en **editores**

Anteriormente unimos los dos conjuntos de datos de tal forma de que el resultado excluyera a quellos personajes que tuvieran unn editor que no aparecía en **editores**, ahora debe hacer lo opuesto, el resultado debe incluir solo aquellos personajes que tienen un editor que no **NO** aparece en **editores**
